...
class MoleculeVAE():
    ...
    def create(self,
        ...
        decoded_mean = lambda z_: TimeDistributed(Dense(charset_length, activation='softmax'), name='decoded_mean')(
            GRU(501, return_sequences = True)(
            GRU(501, return_sequences = True)(
            GRU(501, return_sequences = True)(
            RepeatVector(max_length)(
            Dense(latent_rep_size, name='latent_input', activation = 'relu')(z_))))))

        encoded_input = Input(shape=(latent_rep_size,))

        self.autoencoder = Model(x, decoded_mean(z))
        self.encoder = Model(x, z_mean)
        self.decoder = Model(encoded_input, decoded_mean(encoded_input))
